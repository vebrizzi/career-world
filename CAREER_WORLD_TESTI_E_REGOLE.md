...
- **NPC "persona"** (ruoli `mgr`/`col`): pescano a caso — ma in modo **stabile** (hash dell'id NPC, `npcTextureKey()`/`hashStr()`) — tra **7 avatar** in `AVATAR_KEYS`: `avatar_old_man`, `avatar_old_woman`, `avatar_young_man`, `avatar_bald_guy`, `avatar_afro_woman`, `avatar_black_man`, `avatar_arab_woman` (file in `public/npc-avatars/`). Nessun avatar è legato a un ruolo specifico — scelta intenzionale per non fissare stereotipi (es. "manager" sempre uomo) in un gioco sul bias.
